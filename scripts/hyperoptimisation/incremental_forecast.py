...
def run_forecast():
    ...
    training_names = ['exclude_None-a_garch_0.0-b_garch_0.0_phi_0.5',]
    seasons = ['2026-2027',]         
    ...
    for training_name, season in zip(training_names, seasons):
        ...
        for i, n_observations in enumerate(range(min_obs, max_obs)):
            ...
            for cluster_idx in cluster_indices:
                ...
                # get all data
                _, all_data, all_dt, _, _ = get_NHSN_HRD_data(start_calibrations, modifier_reference_dates, max_obs,
                                                                                type = 'synthetic',
                                                                                name = 'early_season_decrease',
                                                                                forecast_horizon=forecast_horizon,
                                                                                state_fips=state_fips_index['fips_state'].values) # (n_season, n_variables, n_observations)
                ...
                # Visualise goodness-of-fit
                ## Match dates of obs and pred variables in model
                ...
                for s in range(len(state_fips_index)-1):
                    fig,ax=plt.subplots()
                    ## data
                    ax.scatter(all_dt, 7*all_data[0,s,:], marker='o', color='black')
                    ## training
                ...
                hv_out = simout_to_hubverse_admissions(pred,
                                                                    reference_date,
                                                                    dict(zip(state_fips_index["abbreviation_state"],
                                                                    state_fips_index["fips_state"])),
                                                                    quantiles=True)
                hv_out = hv_out[hv_out['horizon'] <= 3] # limit admissions to 4-week aheads
