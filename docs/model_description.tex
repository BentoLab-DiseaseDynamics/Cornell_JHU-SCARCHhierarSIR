...
Spatial correlation of the seasonal mean modifier is modeled using a Leroux-style conditional autogregressive or CAR model \citep{leroux2000}, because it offers great flexibility \citep{lee2011},
\begin{equation}
...
\begin{align}
    ...
    \sigma^2_{i,j}(l) &= \omega_{i,j} + \alpha_{i,j}\,\varepsilon^2_{i,j}(l-1) + \beta_{i,j}\,\sigma^2_{i,j}(l-1),
\end{align}
...
\end{equation}
so that $\bm{v}_i(l) \sim \Normal\big(\bm{0}, \bm{Q}(\psi_2)^{-1}\big)$. Equation~\eqref{eq:ar1} represents the AR(1) dynamics governing deviations from the seasonal mean modifier, with persistence $\phi_{i,j}$. The GARCH(1,1) process governs the time-varying innovation variance, where $\omega_{i,j}$ controls the baseline variance, $\alpha_{i,j}$ captures sensitivity to new shocks, and $\beta_{i,j}$ controls persistence of volatility.\\

The AR(1) persistence $\phi_{i,j}$ and the baseline noise $\omega_{i,j}$ are both modeled as non-centered additive hierarchies across seasons and U.S.\ states and territories with logit-link and log-link functions respectively. Because it is not possible to identify $\alpha_{i,j}$ and $\beta_{i,j}$ simultaneously, we assume $\beta_{i,j} = c $ with $c=0.0$ as the baseline assumption (no volatility clustering) and $\alpha_{i,j} = \alpha$. The initial variance is equal to $\sigma_{i,j}^2 = \omega_{i,j}$.
...
\end{equation}
where the halfnormal prior shrinks the negative binomial to a Poisson distribution, with the weight of $\sigma=0.001$ determined on a trial-and-error basis by assessing the coverage of the 95~\% confidence interval on the training data.\\
...
\end{equation}
alternatives to this normalization function use the average natural logarithm of the case counts or the maximum case count per season and state. The square root had superior performance compared to the natural logarithm, which flattens more aggressively. Posterior sampling is performed using the No-U-Turn Sampler (NUTS) via PyMC.
...
The \texttt{SCARCHhierarSIR} model extends the original \texttt{hierarchSIR} in three important ways. First, instead of treating influenza dynamics across U.S.\ states and territories as spatially independent, we now incorporate spatial correlation in the Bayesian hierarchical inference layer. There is no spatial connection (e.g., mobility) between U.S.\ states and territories through the forward-simulating SIR model.\\
...
The original \texttt{hierarchSIR} model implemented the forward-simulation in \texttt{C++}, with the posterior probability implemented in \texttt{python} and sampled using the ensemble sampler of Goodman and Weare available through \texttt{emcee}. Overall, the computational resources needed to train \texttt{SCARCHhierarSIR} are similar to those of \texttt{hierarchSIR} model, the added value of the refactor lies in the user-friendliness of \texttt{pyMC}, which allows for easier refinements of the Bayesian inference layer compared to a direct implementation in \texttt{python}.\\
...
The model was trained the NHSN HRD lab-confirmed hospital admissions for influenza from the 2023--2024, 2024--2025, and 2025--2026 season, in 52 U.S\ states and territories. Eight independent chains were run for 250 iterations, with the first 50 samples discarded as burn-in. Sampling took 48 hours on an M3 Pro chip (12c).\\

Figure~\ref{fig:CT_2025-2026_goodness-fit} shows the model's output for Connecticut in the 2025--2026 season after training. Most notably, during January and February 2026 the transmission modifiers were significantly below their seasonal mean (second and third panels). The shocks needed to drive the modifiers away from their season mean inflate the system's variance (fourth and fifth panels), showing the mechanism characterizing the AR(1)-GARCH(1,1) dynamics. \\
