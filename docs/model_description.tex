...
\begin{align}\label{eq:f_R}
    ...
    \text{logit}\,\bar{f}_R &\sim \Normal\!\left(\text{logit}(0.4),\ 0.22\right), \nonumber\\
    \sigma^{(i)}_{f_R} &\sim \HalfNormal\!\left(1/5\right), \nonumber\\
    \sigma^{(j)}_{f_R} &\sim \HalfNormal\!\left(1/10\right), \nonumber\\
    u^{(i)}_{f_R}, u^{(j)}_{f_R} & \overset{\mathrm{iid}}{\sim}\Normal(0,1),
\end{align}
The prior on $\text{logit}\,\bar{f}_R$ places the population-mean immunity at $37.5\pm10.0\%$, which results in an effective reproduction number of $R_e = 1 \given \Delta \beta_{i,j}(t) = 0$ at the start of the influenza season, anchoring the model, consistent with \texttt{hierarchSIR}.
