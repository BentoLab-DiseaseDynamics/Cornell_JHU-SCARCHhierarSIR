...
$\Delta \beta_{i,j}(t)$ represents a time-varying multiplicative modifier to the baseline transmission coefficient $\beta_0$, obtained by smoothing a piecewise-continuous trajectory of 35 week-long modifiers $\Delta \mathcal{B}_{i,j}(l)$ with a Gaussian filter ($\sigma = 1~\text{d.}$). For the influenza season of year $\{\text{X}-\text{X}+1\}$, the first modifier represents 7-14 September of year $\text{X}$ and the last modifier represents 1-7 May of year $\text{X}+1$. Outside of this range, there is no modification of the transmission coefficient and hence $\Delta \beta_{i,j}(t)=0$. Simulations are started on August 15 of year $\text{X}$. This approach extends the flexibility of the SIR model, allowing it to better capture complex patterns in the observed incidence data, effectively acting as a discrepancy model in the transmission domain \citep{osthus2019}.
...
\begin{align}
	 \log \left(\rho_{i,j}\right) &= \log \left( \bar{\rho} \right) + \sigma^{(i)}_\rho\, u^{(i)}_{\rho} + \sigma^{(j)}_\rho\, u^{(j)}_{\rho}, \nonumber\\
    \log \left( \bar{\rho} \right)  &\sim \Normal\!\left(\mu_{\rho},(1/2)^2\right), \nonumber\\
    \sigma^{\text{season}}_\rho,\, \sigma^{\text{state}}_\rho &\sim \HalfNormal\! \left( 1/5 \right), \nonumber\\
    u^{(i)}_{\rho}, u^{(j)}_{\rho} & \overset{\mathrm{iid}}{\sim} \Normal(0,1),
\end{align}
where the priors $\mu_{\rho} = \log \left( 1.0~\% \right)$ and $\sigma_{\rho} = 1/2$ center the hierarchy at 1.0~\% case ascertainment with 95~\% of the mass between $0.2~\%$ and $4.5~\%$. The halfnormal distribution on $\sigma_{\rho}^{(i)},~\sigma_{\rho}^{(j)}$ shrink season- and spatial effect sizes when the data does not support them.\\

The initial fraction of infected $f_I$ is small ($f_I$ $\approx 10^{-4}$), and for this reason we use a log-link instead of the more natural logit-link function, because this results in better posterior geometry.\\
...
\begin{align}\label{eq:f_R}
	\text{logit} \left( f_{R,i,j} \right) &= \text{logit} \left(\bar{f}_R\right) + \sigma^{(i)}_{f_R}\, u^{(i)}_{f_R} + \sigma^{(j)}_{f_R}\, u^{(j)}_{f_R}, \nonumber\\
    \text{logit} \left(\bar{f}_R \right) &\sim \Normal\!\left(\text{logit}(0.375),\ 0.22\right), \nonumber\\
    \sigma^{\text{season}}_{f_R} &\sim \HalfNormal\!\left(1/5\right), \nonumber\\
    \sigma^{\text{state}}_{f_R} &\sim \HalfNormal\!\left(1/25\right), \nonumber\\
    u^{(i)}_{f_R}, u^{(j)}_{f_R} & \overset{\mathrm{iid}}{\sim}\Normal(0,1),
\end{align}
The prior on $\text{logit} \left(\bar{f}_R\right)$ places the population-mean immunity at $37.5\pm10.0\%$, which results in an effective reproduction number of $R_e = 1 \given \Delta \beta_{i,j}(t) = 0$ at the start of the influenza season, anchoring the model, consistent with \texttt{hierarchSIR}.

%-----------------------------------------------------------
\subsubsection*{Spatially Correlated AR(2)--GARCH(1,1) Transmission Modifiers}\label{sec:argarch}
%-----------------------------------------------------------
...
\end{equation}
where $\Delta\bar{\mathcal{B}_{j}}(l)$ is the seasonal mean modifier component in U.S.\ state or territory $j$ in week $l$, and $z_{i,j}(l)$ is the corresponding stochastic AR(2)--GARCH(1,1) component \citep{bollerslev1986}, which represents the current week's deviation from the seasonal mean modifier.
...
\begin{equation}
\Delta \bar{\mathcal{B}}_j(l) = \tanh \left( \sum_{k} B_{lk}\zeta_{kj} \right),
\end{equation}
...
The stochastic part $z_{i,j}(l)$ evolves according to a stationary AR(2) model with a repeated real root resulting in a critically damped decay response, whose innovation variance follows a GARCH(1,1) model. For each season $i$, state $j$, and week $l$, the recursion is,
\begin{align}
	z_{i,j}(l) &= 2 \phi \,z_{ij}(l-1) - \phi^2 z_{ij}(l-2) + \varepsilon_{ij}(l), \\ \label{eq:ar1}
	\varepsilon_{ij}(l) &= v_{ij}(l)\,\sigma_{ij}(l), \\
    \sigma^2_{ij}(l) &= \omega_G + \alpha_G \,\varepsilon^2_{ij}(l-1) + \beta_G\,\sigma^2_{ij}(l-1),
\end{align}
with initialization $z_{ij}(0) = z_{ij}(-1) = 0$ and $\varepsilon_{ij}(0) = 0$. The innovation terms $\bm{v}_i(l) = (v_{i1}, \ldots, v_{iS})^T$ driving the system are spatially correlated across U.S.\ states and territories using a Leroux CAR model \citep{leroux2000},
\begin{equation}
...
Equation~\eqref{eq:ar1} represents the AR(2) model governing deviations from the seasonal mean modifier, with persistence $\phi$. The AR(2) model exhibits structural momentum, and for $\phi < 0.5$ it follows an S-shaped decay back to the seasonal mean transmission on the forecast horizon. The GARCH(1,1) process governs the time-varying innovation variance, where $\omega_G$ controls the baseline variance, $\alpha_G$ captures the sensitivity of the system's volatility to new shocks, and $\beta_G$ controls persistence of its volatility.\\
...
For the 2026--2027 forecasting season, however, the available historical data are insufficient to perform this procedure reliably. The hierarchical model requires at least three seasons for training, leaving too few independent seasons for meaningful leave-one-season-out validation. Generating synthetic seasons to augment the validation set is not preferred because the true distribution of plausible influenza seasons is unknown, evolving and auto-regressive, introducing potential bias. In spite, to operationally test our model and make the bare minimum inferences about hyperparameters, we constructed one synthetic season by averaging the available case data and negatively biasing transmission prior to December, and positively biasing it in January and February. Based on this analysis, we fixed the stochastic-process hyperparameters for the 2026--2027 season at $\alpha_G=\beta_G=0$, $\omega_G=0.005$, and $\phi=0.375$. These values result in a homoskedastic autoregressive stochastic component with moderate persistence, which we consider a safe choice for the initial deployment of the model.
...
Third, and most importantly, \texttt{SCARCHhierarSIR} introduces a structured stochastic model for the time-varying transmission modifier. The modifier at each week is represented as the sum of a seasonal mean component and a stochastic deviation from that mean, with the latter generated by a spatially correlated AR(2)--GARCH(1,1) process. The AR(2) component captures the persistence of deviations from the typical seasonal trajectory, allowing an unusually high or low transmission period to influence subsequent weeks. The GARCH(1,1) component allows the magnitude of these stochastic deviations to vary over time, producing greater forecast uncertainty when the trajectory departs substantially from the seasonal mean. This provides a mechanism for the predictive distribution to adapt its width to the magnitude of observed departures from typical seasonal dynamics, which may improve probabilistic forecast performance as measured by the Weighted Interval Score (WIS). The contribution of this stochastic component to forecasting performance will be evaluated prospectively beginning with the 2027--2028 forecasting season. In contrast, the time-varying transmission modifier in \texttt{hierarchSIR} was modeled over a two-week window and subsequently reverted to the seasonal mean, without a persistent stochastic process governing deviations from that mean.

%-----------------------------------------------------------
\section{Training results}
%-----------------------------------------------------------

The model was trained for the 2026--2027 season with the hyperparameters fixed at $\alpha_G=\beta_G=0$, $\omega_G=0.005$, and $\phi=0.375$. A dense mass matrix was used on the parameter sets $\left\{\psi_1, \psi_2 \right\}$, $\left\{ \text{logit} \left( \bar{f_R} \right), \log \left( \bar{f_I} \right), \log \left( \bar{\rho} \right) \right\}$, and $\left\{ \sigma^{\text{season}}_{f_R}, u^{(i)}_{f_R}, \sigma^{\text{season}}_{f_I}, u^{(i)}_{f_I}, \sigma^{\text{season}}_{\rho}, u^{(i)}_{\rho} \right\}$, which converged after 500 tuning steps, after which 500 samples were drawn on 4 independent chains. All $\hat{R} < 1.1$, indicating good sampling quality. Spatial correlation of the mean across-season modifier trajectories $\Delta \bar{\mathcal{B}_j} (l)$ was high ($\psi_1 = 0.83$), while spatial correlation in the within-season stochastic shocks was small but significantly nonzero ($\psi_2 = 0.19$).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{forestplot-alpha_inv.pdf}
    \caption{\textbf{Inverse overdispersion coefficient of the negative binomial observation model.} Values close to zero converge to a Poisson observation model, which happens in states with few cases. Opposed, states with a lot of cases have more overdispersion. The negative binomial observation model therefore alleviates the Poisson model's shrinking relative dispersion at higher means.}
    \label{fig:forestplot_alpha_inv}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{forestplot-fI.pdf}
    \caption{\textbf{Fraction infected at simulation start (Sept 1).} Global, state and season effects.}
    \label{fig:forestplot_fI}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{forestplot-fR.pdf}
    \caption{\textbf{Fraction recovered at simulation start (Sept 1).} Global, state and season effects.}
    \label{fig:forestplot_fR}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{forestplot-rho.pdf}
    \caption{\textbf{Case ascertainment.} Global, state and season effects. }
    \label{fig:forestplot_rho}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{NC_2025-2026_goodness-fit.pdf}
    \caption{\textbf{Goodness-of-fit in North Carolina on the 2025--2026 season.} From top to bottom: 1) Goodness-of-fit to the case data. 2) Across-season mean modifier trajectory in green, within-season modifier trajectory in grey. 3) Stochastic component of the modifier trajectory ($z_{ij}(l)$). 4) Variance of the stochastic model ($\sigma^2_{ij}(l)$). 5) Shocks of the stochastic model ($\varepsilon_{ij}(l)$). Results indicate persistence $\phi$ may be higher than 0.5.}
    \label{fig:goodness-of-fit-NC}
\end{figure}
